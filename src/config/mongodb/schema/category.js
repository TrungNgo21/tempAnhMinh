	...
	},
	{
		statics: {
			async validateDelete(id, product) {
				...
				const isParent = await this.countDocuments({ parentCate: id })
					.lean()
					.exec();
				if (isParent !== 0) {
				...
				const prodCount = await product
					.countDocuments({ category: result._id })
					.lean()
					.exec();
			...
			async validateUpdate(cateDTO, product) {
				const result = await this.find(
					{ name: cateDTO.name, parentCate: cateDTO.parentId },
					'_id'
				)
					.lean()
					.exec();
				...
				const prodCount = await product
					.countDocuments({ category: result[0]._id })
					.lean()
					.exec();
