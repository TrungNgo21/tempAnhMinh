...
class InsertProductDTO {
	constructor(
		...
		this.attribute = new Map(
			attribute.map((element) => [element.name, element.value])
		);
		if (pAttribute != null) {
			this.pAttribute = new Map(
				pAttribute.map((element) => [element.name, element.value])
			);
		} else {
			this.pAttribute = new Map();
		}
	...
	getVolume() {
		return (
			this.dimension.width * this.dimension.length * this.dimension.height
		);
	}
